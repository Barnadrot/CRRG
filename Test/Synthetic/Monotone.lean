...
private theorem tighten : Edge (listBound.goalAt 9) (listBound.goalAt 7) :=
...
private theorem improve9to7 : Progress listBound 9 7 where
...
private theorem improve7to3 : Progress listBound 7 3 where
  improvement := ⟨by omega, by omega⟩
  proof := bound3
private theorem improve9to3 : Progress listBound 9 3 := improve9to7.trans improve7to3
...
#expect_failure
private theorem notProgress : Progress listBound 7 7 where
...
#expect_failure
private theorem backwards : Progress listBound 7 9 where
  improvement := ⟨by omega, by omega⟩
  proof := bound9
...
#expect_failure
private theorem unprovenImprovement : Progress listBound 9 2 where
  improvement := ⟨by omega, by omega⟩
  proof := bound7
...
private theorem momentProgress : Progress momentBound 3 5 where
  improvement := ⟨by omega, by omega⟩
  proof := fun _ hn => hn
...
#expect_failure
private theorem exchangeRate : Progress listBound 3 5 := momentProgress

/- Negative: there is no operation combining progress across families. -/
#expect_failure
private theorem combined : Progress listBound 9 3 := improve9to7.trans momentProgress
